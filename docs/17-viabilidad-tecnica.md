...
| I3 | ~~Totales de cartera y gastos se suman en el cliente con `.limit()` fijo~~ **Resuelto 2026-09-25**: los totales los calcula la base (`cartera_totales`, `gastos_totales`, `gastos_por_categoria` en `supabase/migrations/20260925100000_totales_en_base.sql`), sobre todas las filas; la lista sigue con tope y lo dice | `src/features/billing/queries.ts`, `src/features/finance/queries.ts`, `AdminHome.tsx`, `ExpensesPage.tsx` | `supabase/tests/totales.sql` (28 aserciones: 251 cobros y 350 gastos, más que los topes de la lista; "hoy" en Bogotá y no en UTC; ceros para otro box y para coach sin finanzas). Verificado en vivo en widawi contra sumas SQL directas | — |
| I4 | ~~`useMonthlyCollected` sin `.limit()` pero sin paginar~~ **Resuelto 2026-09-25**: `recaudo_mes` en la base, con el mes en la zona horaria del box (un pago a las 21:00 del 30 en Bogotá ya no se va al mes siguiente) | `src/features/billing/queries.ts` | Misma prueba de arriba | — |
...
| 9 | ~~Mover el `sum()` de cartera/gastos a la base (I3/I4)~~ **Hecho 2026-09-25** | Importante | — | — |
