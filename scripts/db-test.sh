...
if out="$("${PSQL[@]}" -v ON_ERROR_STOP=1 -f supabase/tests/self_signup.sql 2>&1)"; then
  ...
  echo "$out"; echo "✗ Falló la prueba de registro de boxes"; exit 1
fi
...
if out="$("${PSQL[@]}" -v ON_ERROR_STOP=1 -f supabase/tests/automations.sql 2>&1)"; then
  ...
  echo "$out"; echo "✗ Fallaron las pruebas de automatizaciones"; exit 1
fi

echo "→ Plataforma (alta de boxes, superadmin, suspensión)"
if out="$("${PSQL[@]}" -v ON_ERROR_STOP=1 -f supabase/tests/saas.sql 2>&1)"; then
  echo "$out" | grep -E "ok ·|PLATAFORMA|SAAS" || { echo "$out"; echo "✗ Sin aserciones"; exit 1; }
else
  echo "$out"; echo "✗ Fallaron las pruebas de plataforma"; exit 1
fi
