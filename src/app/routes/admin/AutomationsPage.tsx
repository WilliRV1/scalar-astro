...
export default function AutomationsPage() {
  ...
  const { data: ajustes } = useAjustesAutomatizacion(orgId);
  ...
  const { data: bitacora, isLoading: cargandoBitacora } = useBitacora(orgId, {
    estado: filtro,
    busqueda: busqueda.trim() || undefined,
  });

  const simulacion = ajustes?.simulation_mode ?? true;
  ...
  return (
    <div className="space-y-4">
      ...
      {error && <ErrorNote>No se pudieron cargar las reglas. Vuelve a intentarlo.</ErrorNote>}
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            ...
            className={`relative h-10 w-20 shrink-0 border transition ${
              simulacion ? 'border-amber-400 bg-amber-400/30' : 'border-gray-600 bg-black/40'
            }`}
          >
            <span
              className={`absolute top-1 h-8 w-8 transition-all ${
      ...
      <div className="pt-4">
        ...
        <div className="mt-3 space-y-2">
          ...
          {!cargandoBitacora && (bitacora?.length ?? 0) === 0 && (
            <EmptyState
    ...
    <Card className={regla.is_active ? '' : 'opacity-60'}>
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className={`relative h-7 w-14 shrink-0 border transition ${
        ...
        >
          <span
            className={`absolute top-0.5 h-5 w-5 transition-all ${
              regla.is_active ? 'left-8 bg-primary' : 'left-0.5 bg-gray-500'
            }`}
          />
        </button>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        <span className="text-[10px] uppercase tracking-widest text-gray-600">
          {regla.last_run_at
            ? `Última evaluación: ${new Date(regla.last_run_at).toLocaleString('es-CO')}`
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="shrink-0 text-right">
          ...
          <p className="text-[10px] uppercase tracking-widest text-gray-600">
            {new Date(mensaje.sent_at ?? mensaje.scheduled_for).toLocaleString('es-CO')}
          </p>
          {cancelable && orgId && (
            <button
              type="button"
              ...
              className="mt-1 text-[11px] font-bold uppercase text-gray-500 hover:text-primary"
            >
              No mandarlo
            </button>
          )}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {guardar.isError && <ErrorNote>No se pudo guardar. Vuelve a intentarlo.</ErrorNote>}
      </div>
    </Drawer>
  );
}
